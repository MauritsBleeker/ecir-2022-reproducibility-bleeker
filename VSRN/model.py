...
from VSRN.GCN_lib.Rs_GCN import Rs_GCN

import VSRN.opts
import VSRN.misc.utils as utils
import torch.optim as optim

# from misc.rewards import get_self_critical_reward, init_cider_scorer
from VSRN.models import DecoderRNN, EncoderRNN, S2VTAttModel, S2VTModel
...
class VSRN(object):
    ...
    def __init__(self, opt):
        ...
        # Loss and Optimizer
        if opt.criterion == 'triplet':
            self.criterion = Triplet(margin=opt.margin,
                                     measure=opt.measure,
                                     max_violation=opt.max_violation)

        elif opt.criterion == 'ntxent':
            self.criterion = NTXent(tau=opt.tau)

        elif opt.criterion == "ranking":
            self.criterion = SmoothAP(n=opt.n_sp, tau=opt.tau)

        else:
            raise Exception("No loss function")
    ...
    def train_emb(self, images, captions, lengths, ids, *args):
        ...
        wandb.log({'Le': loss.data}, step=self.Eiters)
