...
class PrecompDataset(data.Dataset):
    ...
    def __init__(self, data_path, data_split, vocab, opt, captions_per_img=5):
        self.vocab = vocab
        loc = data_path + '/'
        ...
        self.ranking_based = opt.ranking_based and data_split == 'train'
    ...
    def __getitem__(self, index):
        ...
        if self.ranking_based:
            img_id = index
            captions = self.captions[index * 5:index * 5 + 5]

            if len(captions) > self.n_sp:
                captions = random.sample(captions, self.n_sp)
        ...
        if self.ranking_based:
            ...
            targets = self.process_caption(captions)
...
def get_precomp_loader(data_path, data_split, vocab, opt, batch_size=100,
    ...
    data_loader = torch.utils.data.DataLoader(dataset=dset,
                                              ...
                                              shuffle=shuffle,
                                              pin_memory=True,
                                              collate_fn=partial(collate_fn, ranking_based=opt.ranking_based  and data_split == 'train'))
    return data_loader
